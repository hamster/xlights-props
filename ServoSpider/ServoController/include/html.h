    ...
    <div id="settings-tab" class="tab-content">
      ...
      <div class="status-box">
        ...
        <form onsubmit="return handleFormSubmit(event, '/save-stepper')">
          ...
          <div class="form-group" id="lookaheadFieldsGroup">
            <label for="stepperLookaheadSteps">Lookahead Distance (steps):</label>
            <input type="number" id="stepperLookaheadSteps" name="stepperLookaheadSteps" value="{{STEPPER_LOOKAHEAD_STEPS}}" min="1" max="1000000" required>
            <p style="color: #666; font-size: 12px; margin: 4px 0 0;">Must exceed the tracking profile's worst-case stopping distance (Tracking Speed&sup2; / (2 &times; Tracking Acceleration)) or the ramp generator can still catch up to the extended target and decelerate anyway.</p>
            <label for="stepperLookaheadSettle" style="margin-top: 8px;">Lookahead Settle Time (ms):</label>
            <input type="number" id="stepperLookaheadSettle" name="stepperLookaheadSettle" value="{{STEPPER_LOOKAHEAD_SETTLE}}" min="20" max="5000" required>
          </div>
