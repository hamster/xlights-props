...
- **Tracking Motion Strategy**: **Direct** (`moveTo()` on every DDP packet — the compiled-default fallback for any device that has never saved this setting) or **PID** (closed-loop control with velocity feedforward — see CLAUDE.md's "Stepper Tracking Modes" for the full design and TODO.md for the tuning history). As of 2026-09-07 this is the only choice offered here — Coalesce/Streaming/Lookahead remain implemented but are no longer exposed in this dropdown, and their bench-testing status varies a lot: Coalesce was validated once, early on, and performed nearly identically to Direct; Streaming was bench-tested and found unstable (repeated stalls/freezes), which is why it's shelved; Lookahead was never bench-tested at all - implemented, never run on real hardware. All three are still reachable via the `$SET`/`GET /tunable` bench protocol (`trackMode=1/2/3`) if ever needed again.
