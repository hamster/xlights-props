...
- **Tracking Motion Strategy**: **Direct** (`moveTo()` on every DDP packet — the compiled-default fallback for any device that has never saved this setting) or **PID** (closed-loop control with velocity feedforward — see CLAUDE.md's "Stepper Tracking Modes" for the full design and TODO.md for the tuning history). As of 2026-09-07 this is the only choice offered here — Coalesce/Streaming/Lookahead (earlier bench-tested alternatives to Direct, Streaming shelved as unstable) remain implemented but are no longer exposed in this dropdown; they're still reachable via the `$SET`/`GET /tunable` bench protocol (`trackMode=1/2/3`) if ever needed again.
