...
- **Tracking Motion Strategy** (experimental — Direct / Coalesce / Streaming): three different ways of turning DDP updates into motor commands, added after log analysis showed most of the remaining jerkiness comes from `moveTo()` always planning a full stop at each new nearby target, not from accel/speed tuning alone. Direct is the original behavior; Coalesce batches small updates into fewer, larger moves (tunable window/step-size); Streaming runs continuously at an estimated speed instead of stopping at each target, snapping to the exact position once updates go quiet (tunable rate window/settle time). Not yet bench-validated — pick one and compare against the others using the Compact Motion Log described below.
