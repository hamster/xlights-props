...
- **Reconnect Retry Interval** (seconds, default 20, 0=disabled) - how often the device checks its WiFi connection and retries as a client with saved credentials. Applies in both directions: if it's already connected as a client and the connection drops, it retries on this cadence (after a few consecutive failed checks, to avoid overreacting to a momentary blip); if it fell back to broadcasting its own Access Point after a failed boot-time connect, it also periodically retries as a client on this same cadence, in the background, without dropping the AP - so it's still reachable locally the whole time, and automatically returns to normal client operation (dropping the AP) once the real network is reachable again, no manual "Connect Now" or reboot needed. 0 disables the periodic check entirely in both cases - a dropped/failed connection then only recovers via "Connect Now" or a reboot. See TODO.md for the still-open work on an explicit Client-only/AP-fallback/AP-only mode selector, which is a related but separate piece.
