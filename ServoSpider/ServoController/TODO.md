...
- [ ] `CHOPCONF.hstrt`/`hend` (SpreadCycle hysteresis start/end) are still untouched (defaulting to 0/0 in the shadow). This is a functionally valid, just conservative, SpreadCycle configuration - not confirmed broken, but worth listening for excess coil noise/ripple and tuning if so, since 0/0 isn't necessarily what the chip's true reset default would have been.
- [ ] Microstep resolution is now hardcoded to 16 in `applyTmcSettings()` (`tmcDriver->microsteps(16)`) rather than exposed as a Settings field - fine for now, but note that changing it later requires a re-home (bottomPosition is measured in actual steps, so it self-corrects, but the previously-tuned Stepper Speed (Hz) will feel different since physical distance per step changes).
