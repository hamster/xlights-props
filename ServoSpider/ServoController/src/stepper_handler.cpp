...
// IF the switch it triggered at bootup, try to move a little bit to move off the switch first
bool home(){

  int counter = 0;
  int direction = 1;
  int readyToHome = true;
  ...
  if(digitalRead(homingSwitchPin) == HIGH){
    // Homing switch is triggered at startup
    Serial.println("Homing switch triggered at bootup, moving off switch...");

    readyToHome = false;

    // Which way to go?  Step forward a little to see if the switch opens
    Serial.println("Moving forward...");
    for(int i = 0; i < 25; i++){
      stepper->moveTo(10 * i);
      delay(100);
      if(digitalRead(homingSwitchPin) == LOW){
        // OK, we're good.  Move a little more so homing doesn't stall
        stepper->move(2500);
        readyToHome = true;
    ...
    if(readyToHome == false){
      // No luck, try moving backward a little bit
      Serial.println("Didn't clear, moving backward...");
    ...
    Serial.println("Moved off homing switch");
  }

  Serial.print("Moving to find initial homing position..");
  interruptTriggered = false;
  delay(500);
  stepper->runBackward();
  ...
  while(1){
    ...
    if(direction == 1){
      Serial.print(".");
      // Still looking for initial position
      ...
      // Looking for the other end of the wheel
      Serial.print(".");
      if(interruptTriggered){
        // Found it.
        Serial.println();
        ...
        bottomPosition = stepper->getCurrentPosition() / 2;
        interruptTriggered = false;
    ...
    counter++;
    if(counter > 60){
      // Waited 30 seconds and still not homed
      stepper->forceStop();
      Serial.println("ERROR timed out while homing!");
      stepper->setAcceleration(stepperAccel);
      stepper->setSpeedInHz(stepperSpeed);
      return false;
    }
